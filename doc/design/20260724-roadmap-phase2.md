...
- **M2. 관리 대시보드 리디자인** — 현재 대시보드(:8091)를 운영·검증에 맞게 재설계. M1에서 이해한 근거(관계·출처)·품질 지표 노출. `/design`에서 화면 설계.
- **M3. 필요 기능 추가** — M1·M2 탐색에서 도출.
- **M4. YouTube + 인증 연결** — Supabase 인증 활성화(게이트웨이 JWKS 코드 구현됨, ADR 0007) + 대시보드 무인증 노출 정리 + YouTube 자동 발행(`publishing/youtube.py` 스텁→OAuth, 승인 게이트 통과 후).
...
- **저장 = 전용 `admin` 서비스 + `admin_db` 신설(확정)**:
  - 테이블(예): `stock_master`(코스피200 시드 — 코드 46 승격) · `watch_stocks`(기본 전체 ON) · `watch_keywords` · `collection_sources` · `collection_period`.

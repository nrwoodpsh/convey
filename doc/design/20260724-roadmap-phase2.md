...
- **M2. 관리 대시보드 리디자인** — **[구현 ✅ ㊵ P1·P2]** `content/20260725-task-dashboard-redesign.md`. 탭(오늘자·쇼츠·설정)+설정(admin 중계)+근거·품질 지표. 백엔드·프런트 완료.
- **M3. 필요 기능 추가** — **[M1 개선 설계에 흡수, 구현 ✅]** 온톨로지(㉟)·템플릿(㊳)·수집(㉝)·배경(㊴) 전부 구현.
- **M4. YouTube + 인증 연결** — **[코드 ✅ / 실연결 미완 ㊶]** `publishing/20260725-task-publish-youtube.md`. 발행·인증·멱등·폴백 코드 완료. **실 YouTube 업로드(`YOUTUBE_REFRESH_TOKEN`)·실 Supabase(`SUPABASE_URL`)는 미연결** — 2차 점검 후 진행(사람 자격증명). 대시보드=무인증 로컬 유지(ADR 0010).
...
|:--|:--|:--:|:--:|:--:|:--|
| 1 | 전체 아키텍처 | ✅ | — | — | 지도 |
| 2 | 기사 수집 기준 | ✅ | ✅ | ✅ | 오태깅 수정(㉜) 구현 완료 |
| 3 | 엔티티(노드) 추출 | ✅ | ✅ | ✅ | 타입검증(㉞)→㉟ 온톨로지에 흡수·구현 |
| 4 | 릴레이션 규칙 | ✅ | ✅ | ✅ | ㉟(온톨로지+domain/range) 구현 |
| 5 | Kafka 이벤트 흐름 | ✅ | ✅ | ✅ | 정석 스택 ㊱ P1~P5 구현(봉투·Avro/Registry·Debezium·DLQ·Grafana). P4·P5는 커밋 대기 |
| 6 | GraphRAG 회수 & 이슈 선별 | ✅ | ✅ | ✅ | news_count 중복(㊲) + 시나리오 템플릿(㊳) + 이슈 게이팅(㉝) |
| 7 | 영상 생성 규칙 | ✅ | ✅ | ✅ | 배경 전략(㊴ P1·P2)+가독성 패널 구현 |
| 8 | 게이트웨이·보안 | ✅ | ✅ | ◐ | 인증(JWKS/HMAC) 학습·㊶ 코드 완료. **실 Supabase 연결 미완**(2차 점검 후) |

> 상태: ☐ 미착수 / ▶ 진행 / ◐ 코드완료·실연결미완 / ✅ 완료. 항목 진행하며 갱신.
>
> **2차 개발 종합(2026-07-27)**: 상품화(Q·U·C) 8라운드 커밋 완료 · 학습 트랙 ㊱ P1~P3 커밋·P4·P5 커밋 대기 · ㊶ 실 외부연결(YouTube·Supabase) 2건 미완(2차 점검 후). 통합 스모크 = `doc/ref/integration-scenario-phase2.md`.

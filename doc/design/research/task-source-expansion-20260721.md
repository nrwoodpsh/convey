...
| 20260721 | /builder(리뷰 수정) | `/review` 발견 반영 — **[Medium]** news-feed worker의 동기 fetch(RSS·Naver·DART·ECOS·FRED)를 `asyncio.to_thread`+`gather`로 감쌈(이벤트 루프 비블로킹 + 병렬 수집, market-feed 패턴 일관). **[Low]** `handle_macro` `as_of` 폴백을 `datetime.now(timezone.utc)`로(naive→tz-aware). mypy clean, to_thread 스모크 검증(naver/dart/ecos/fred 정상). Naver 관련성 필터·ECOS 키 위치는 의도적 무변경(커버리지·이미 미로깅). |

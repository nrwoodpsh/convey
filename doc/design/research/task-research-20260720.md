...
| 20260721 | /builder(라운드⑤ 지원) · Deviation | **가격 근거 공급** — `/research/search`에 optional `price`(PriceEvidence: 최신 종가·직전대비 등락률·시계열, 출처 동반) 부착. `ticker` 파라미터 추가, `repository.latest_price`(최근 N개 종가→시계열·등락률). **계약 이탈**: `SearchRes.price` 추가. 실 research_db 검증(005930 close·등락률·30점 시계열). 스크립트·차트에 사실 수치 공급(알파1·3). |
| 20260721 | /builder(Alembic 베이스라인) | 베이스라인 마이그레이션 `60c1465028e2`(articles·price_ticks) 신설 + `script.py.mako` 스캐폴딩 보강. **`PriceTick`에 `UniqueConstraint(ticker, ts)` 추가** — 앱 레벨 멱등을 DB 제약으로 승격. 빈 temp DB `upgrade head`로 스키마·UNIQUE 생성 검증(실 dev DB 불변). → `doc/summary/20260721-summary-alembic-baseline.md` |
| 20260721 | /builder | **`market.ticks` 소비자 신설** — `handle_tick`(이벤트→PriceTick 저장)·`run_tick_consumer`(Kafka 루프, group `research-ticks`), main 백그라운드 등록. `upsert_price_tick`으로 **멱등 저장**(같은 `(ticker, ts)`면 OHLCV만 갱신 — market-feed가 같은 일봉 반복 발행하므로 중복 방지). 시세는 그래프/LLM 미경유(사실만·수치 조작 금지). **실 KRX tick → 실 research_db 검증**(insert→같은 키 재발행→행 1개 유지·close 갱신). mypy clean. (멱등은 앱 레벨 — `(ticker,ts)` unique 제약·Alembic 베이스라인은 후속.) |

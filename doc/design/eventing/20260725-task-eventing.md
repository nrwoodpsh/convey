...
- **Acceptance Criteria**(Phase별):
  ...
  - [x] AC4(P4): 소비 **수동커밋** + 실패 **RETRY_MAX 재시도 후 DLQ**(`*.dlq`). **검증: 처리 실패 메시지가 재시도 후 dlq 토픽으로, offset은 성공분만 전진.** — 완료(`consume_reliable`, `consume_forever`가 위임 → 전 소비자 무변경 적용). 수동커밋(enable.auto.commit=false, 처리/DLQ 후 commit), 실패 retry_max 재시도 후 `{topic}.dlq` 발행(원본+실패메타). 라이브: 실패 핸들러→DLQ 도달(attempts·error·source_topic)·전 서비스 reliable 모드 소비 에러 0·그래프 노드 증가(파이프라인 라이브).
  - [x] AC5(P5): **Prometheus+Grafana+kafka-exporter**로 consumer_lag 등 대시보드. **검증: Grafana에서 lag 그래프 확인.** — 완료. 라이브: kafka-exporter target UP·Prometheus consumer_lag 22 시계열(실 컨슈머 그룹)·Grafana healthy + 대시보드 "CONVEY Eventing" 3패널(consumer_lag·throughput·dlq_rate) 프로비저닝·dlq_rate 소스(p4 DLQ 토픽 관측).
  - [x] AC6(전체): 가드레일 — 앱 멱등 유지, 무출처 0·로컬 Ollama 불변. mypy·계약·기존 테스트 통과. — enable.idempotence·앱 멱등(source_url) 유지. 각 Phase mypy strict 0·계약 0·회귀 통과.
...
| 일시 | 단계 | 내용 |
|:---|:---|:---|
| 20260726 | /builder(P5) | **㊱ P5 모니터링(Prometheus/Grafana/kafka-exporter)** — 인프라·설정만. compose: `kafka-exporter`(danielqsj, 9308)·`prometheus`(scrape kafka-exporter)·`grafana`(datasource+대시보드 자동 프로비저닝, 익명 열람). `infra/monitoring/`: prometheus.yml·grafana provisioning(datasource·dashboards)·대시보드 JSON(consumer_lag·throughput·dlq_rate 3패널). .env.example 포트. **검증**(설정 유효성+데이터경로): 대시보드 JSON·yaml 유효·kafka-exporter consumer_lag 노출·Prometheus target UP+lag 22시계열·Grafana healthy+대시보드 3패널 프로비저닝·dlq_rate 소스(p4 DLQ 토픽 offset). **㊱ P1~P5 전부 완료** — 봉투·Avro/Registry·Debezium 아웃박스·DLQ/수동커밋·모니터링. |
| 20260726 | /builder(P4) | **㊱ P4 DLQ + 수동커밋**(인프라 추가 없음). `libs/common/kafka.py`: `consume_reliable`(수동커밋 enable.auto.commit=false·성공/DLQ 후 `_commit_sync`·실패 retry_max 재시도 선형백오프 후 `{topic}{dlq_suffix}` 발행)·`_process_one`(디코딩 실패도 DLQ)·`_to_dlq`(원본 payload+error+attempts+failed_at+source_topic). `consume_forever`가 `consume_reliable`에 위임 → 전 소비자(research·publishing·content·video-assembly·issue-detector) 무변경 적용. RETRY_MAX/DLQ_SUFFIX 상수. **검증**: 단위 3(성공→DLQ없음·재시도후 DLQ·디코딩실패→DLQ)+회귀 15·mypy strict 0(commit 오버로드는 `_commit_sync` 헬퍼). 실스택(전 서비스 재빌드): 실패 핸들러→`p4.dlq.test2.dlq` 도달(attempts=2·메타)·전 서비스 reliable 소비 에러 0·그래프 노드 증가. **범위**: P5 Prometheus/Grafana는 후속. |

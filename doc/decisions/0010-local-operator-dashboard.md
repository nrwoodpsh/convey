...
- **M4 확정(㊶ AC5, 2026-07-26)**: 게이트웨이 JWKS 인증 코드는 활성(ADR 0007, `test_supabase_auth` 7건 통과). 대시보드 노출은 **개발=로컬 전용 유지(현행)**, **운영 배포=게이트웨이+Supabase 뒤로**(리버스 프록시/게이트웨이 라우트 + 로그인 게이트)로 확정. 실 Supabase 프로젝트 키 주입은 사람 게이트. 발행(㊶ C1)은 대시보드 "발행" 버튼(사람 클릭) → `content.approved` → publishing만 트리거(자동 발행 금지).

...
> **갱신([0004]·[0005])**: pgvector는 **주력 → 보조**(테마 의미검색). 관계형은 시세·기사 등 **사실** 저장. 관계·인과는 **Neo4j 지식 그래프**([0005] — 이전 "그래프 미사용"을 대체). pgvector(Postgres) 재사용 결정 자체는 유효.

- **상태**: 채택 (2026-07-18)
- **맥락**: CONVEY의 핵심은 "리서치 원문 검색(RAG)"이나 원형은 `agent/rag/{retriever,embeddings}.py`가 **스텁**이고 `main.py`에서 `retriever=None` 주입, `docker-compose.yml`에 벡터DB 없음(pgvector 주석처리). 저장소를 결정해야 함(설계 문답 Q2).

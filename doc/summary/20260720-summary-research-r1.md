...
- `services/research/app/main.py`·`config.py` — Neo4j 드라이버 배선 + consumer 백그라운드
- `services/research/app/consumer.py` — `handle_ingested`(이벤트→Article 저장+LLM 추출+그래프 upsert) + `run_consumer`
- `services/news-feed/app/tagging.py` — `tag_entity_names`(관계추출 allowed 이름)
- `doc/design/research/api-contract.py` — `ResearchIngestedEvent`에 title·body·entities(Deviation)
...
- **라운드① 코어 완료**: 규칙 태깅·LLM 추출·그래프·사실·`/search`·consumer `handle_ingested` 전부 실서비스 검증.
- **남음(후속)**: `run_consumer` 전송 e2e(Kafka 호스트노출 + llm-inference 기동 필요) · 타입 노드(Stock/Sector, 지금 generic Entity) · news-feed/market-feed 실수집(KIS·RSS).
